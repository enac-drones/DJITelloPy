...
class Logger(object):
    ...
    def __init__(self,
        ...
        self.controls = np.zeros((num_drones, 12, duration_sec*self.LOGGING_FREQ_HZ)) #### 12 control targets: pos_x,
                                                                                                             ...
                                                                                                             # ang_vel_y,
                                                                                                             # ang_vel_z
        self.trajectories = np.zeros((num_drones, self.traj_nr , self.traj_len, 6, duration_sec*self.LOGGING_FREQ_HZ))  ## Needs to be defined coreectly
    ...
    def log(self,
        ...
        if current_counter >= self.timestamps.shape[1]:
            ...
            self.trajectories = np.concatenate((self.trajectories, np.zeros((self.NUM_DRONES, len(trajectory), self.traj_len, 6, 1))), axis=4)
        ...
        self.trajectories[drone, :, :, :, current_counter] = trajectory
    ...
    def save(self, flight_type='simulation'):
        ...
        with open(os.path.dirname(os.path.abspath(__file__))+"/logs/save-"+flight_type+"-flight-"+datetime.now().strftime("%m.%d.%Y_%H.%M.%S")+".npy", 'wb') as out_file:
            np.savez(out_file, timestamps=self.timestamps, states=self.states, controls=self.controls, trajectories=self.trajectories)
        print('Saved the flight log !')
