...
def receiveMarkerSet(name, posList):
    ...
    if re.fullmatch(args.name, name) is not None:
        ...
        if send:
            ...
            points = [(pos[X_AXIS], Y_SIGN*pos[Y_AXIS]) for pos in posList] # Turn into 3D here if needed !!!!
            hull = ConvexHull(points)

            # print(name)
            # print([[points[i][0], points[i][1]] for i in hull.vertices])
            markerset[name]['hull'] = [[points[i][0], points[i][1]] for i in hull.vertices]
